    ...
    >
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '48px 32px 80px' }}>
        ...
        <Section number="8" title="Parametric Dimensions (CAD)">
          ...
          <Tip>For real floorplans use the <B>Floorplan</B> node: draw several disconnected wall runs in one node (press <B>Enter</B> or double-click to finish a wall and start the next), then flip the viewport toolbar from <B>Draw</B> to <B>Dimension</B> to place dimensions <B>right inside the node</B> — typing a new value drives the walls. Floorplans read in <B>meters</B>: set <B>Screen Units / Meter</B> and a <B>Drawing Scale</B> (1:50, 1:100…) so a 400px wall reads, say, 4.00 m.</Tip>
          <Tip>Switch the Floorplan toolbar to <B>Elements</B> to drop <B>doors, windows and openings</B> onto a wall. They're hosted on the wall, so they slide and rotate when you dimension it. Each cuts a real opening; drag the marker to slide, double-click to set the <B>width in meters</B>, and use the door's rotate handle to flip hinge/swing.</Tip>
